...
export default function Products() {
    ...
    const [filters, setFilters] = useState<Record<string, Set<string | number | {min : string; max : string}>>>({});

    function updateFilters(type: string, checked: boolean, value: string | { min: string; max: string }) {
    ...
    const filteredProducts = products.filter((product: Product) => {
        ...
        return Object.entries(filters).every(([key, value]) => {
            ...
            if (key === "price") {
                const range = Array.from(value).find((v) => typeof v === "object") as { min: string; max: string };
                const minPrice = range ? parseFloat(range?.min || "0") : 0;
                const maxPrice = range ? parseFloat(range?.max || Infinity.toString()) : Infinity;

                console.log("minprice: ", minPrice, "maxprice: ", maxPrice, "productprice: ", product.price)
